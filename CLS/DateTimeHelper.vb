...
#Region "Declares"
	...
	Private Declare Function FileTimeToSystemTime Lib "Kernel32.dll" (ByRef lpFileTime As FileTime, ByRef lpSystemTime As SYSTEMTIME) As Integer

	'UPGRADE_WARNING: Structure SYSTEMTIME may require marshalling attributes to be passed as an argument in this Declare statement. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="C429C3A5-5D47-4CD9-8F51-74A1616405DC"'
	Private Declare Function SystemTimeToVariantTime Lib "OleAut32.dll" (ByRef lpSystemTime As SYSTEMTIME, ByRef vbtime As Date) As Integer
...
#Region "Methods - Public"
	...
	''' <summary>
	''' Convert a Win32 FILETIME structure to a VB Date
	''' </summary>
	''' <param name="ft">Win32 API FILETIME that should be converted</param>
	''' <returns>
	''' ft converted to a VB date, CDate(0) in case of an error
	''' </returns>
	Public Function FileTime2VBDate(ByRef ft As FileTime) As Date
		...
		If lRet <> 0 Then
			...
			If lRet <> 0 Then
				Return dtm
			Else
				' Debug.Print("SystemTimeToVariantTime Err: " & CommonHelper.GetLastApiDllStr(lDllErr))
				Return System.DateTime.FromOADate(0)
			End If
		Else
			' Debug.Print("FileTimeToSystemTime Err: " & CommonHelper.GetLastApiDllStr(lDllErr))
			Return System.DateTime.FromOADate(0)
		End If
